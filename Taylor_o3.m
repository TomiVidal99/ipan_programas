...
function [w,El,Ea]= Taylor_o3(h,x,y,f,f1,f2,it)
...
for i=1:it
    ...
    w(i+1)=w(i)+h*f(x(i),w(i))+((h^2)/2)*f1(x(i),w(i))+((h^3)/factorial(3))*f2(x(i),w(i));
...
for i=1:it
    ...
    El(i+1)=abs(y(x(i+1))-(y(x(i))+h*f(x(i),y(x(i)))+((h^2)/2)*f1(x(i),y(x(i)))+((h^3)/factorial(3))*f2(x(i),y(x(i)))));
