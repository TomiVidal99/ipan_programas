    ...
    if (c != r)
        ...
        do
            ...
            if (pivot == 0)
                ...
                if ( r == 2)
                  permutated_matrix = A([2, 1], :); # defino la matriz permutada
                  A = permutated_matrix; # intercambio las matrices  
                elseif ( (k+2) < r  && (k-1) != 0)
                  permutated_matrix = A([1:(k-1), (k+1), k ,k+2:r], :); # defino la matriz permutada
                  A = permutated_matrix; # intercambio las matrices  
                elseif ( (k+2) < r  && (k-1) == 0)
                  permutated_matrix = A([1, (k+1), k ,k+2:r], :); # defino la matriz permutada
                  A = permutated_matrix; # intercambio las matrices  
                elseif ( (k+1) == r )
                  permutated_matrix = A([1:(k-1), r, k], :); # defino la matriz permutada
                  A = permutated_matrix; # intercambio las matrices  
                elseif ( k == r )
                  permutated_matrix = A([r, 2:(r-1), 1], :); # defino la matriz permutada
                  A = permutated_matrix; # intercambio las matrices  
                else
                  error_code = 7;
                  disp("CODE ERROR 1");
